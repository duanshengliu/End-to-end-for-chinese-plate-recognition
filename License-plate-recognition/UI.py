...
class Window:
    ...
    def load_show_img(self):
        ...
        if img_open.size[0]*img_open.size[1]>512*512:#图片过大，resize为(512,512)进行显示
            img_open=img_open.resize(512,512)
    ...
    def display(self):
        if self.img_src_path == None:  # 还没选择图片就进行预测
            ...
            if Lic_pred:
                ...
                self.can_src.create_image(256, 256, image=self.img_Tk, anchor='center')  # img_src_copy上绘制出了定位的车牌轮廓,将其显示在画板上
