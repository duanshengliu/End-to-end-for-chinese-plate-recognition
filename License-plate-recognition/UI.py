...
class Window:
    ...
    def load_show_img(self):
        ...
        img_open = Image.open(self.img_src_path).resize((512, 512))
        self.img_Tk = ImageTk.PhotoImage(img_open)
        self.can_src.create_image(3, 3, image=self.img_Tk, anchor='nw')
    ...
    def display(self):
        if self.img_src_path == None:  # 还没选择图片就进行预测
            ...
            if h * w <= 240 * 80 and 2 <= w / h <= 5:  # 满足下面条件说明可能整个图片就是一张车牌
                lic = cv2.resize(img_src, dsize=(240, 80))[:, :, :3]  # 直接resize为(240,80)
                Lic_img = [lic]
            else:
                img_src, img_mask = unet_predict(self.unet, self.img_src_path)  # 通过unet对img_src原图进行预测，得到img_mask
                Lic_img = locate_and_correct(img_src, img_mask)  # 利用core.py中的locate_and_correct函数进行车牌定位和矫正
